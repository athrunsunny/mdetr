...
def create_positive_map(tokenized, tokens_positive):
    ...
    # 这段代码的主要作用是构建一个 positive_map，用于标识哪些 token 与哪些正样本（positive samples）相关联。这在自然语言处理（NLP）的任务中非常常见，尤其是处理中字符和token的匹配关系时。
    # 这个函数的作用主要是，将字符位置映射到 token 位置，并构建一个表示 token 与正样本关联关系的矩阵 positive_map。这个矩阵后续可以用来指导模型训练或者进行正样本相关的分析。
    # 这个函数的输入有两个参数：
    # tokenized: 这通常是一个已经过分词处理的文本对象，比如一个 Hugging Face tokenizer 的输出。
    # tokens_positive: 这是一个包含多个正样本的列表，每个正样本对应文本中的一个字符范围，表示和这些 token 相关联的字符片段（通常是一个二维数组）。
    # 这里我们初始化了一个二维的零矩阵 positive_map，行数等于 tokens_positive 的长度，列数固定为 256。这个矩阵的作用是表示每个正样本与第几个 token 关联。
    positive_map = torch.zeros((len(tokens_positive), 256), dtype=torch.float)
    # 1外层循环：遍历每一个正样本 tok_list。
    # 2中层循环：遍历每一个正样本的起始和结束字符位置 (beg, end)。
    # 3字符位置到 token 位置转换：使用 tokenized.char_to_token 函数将字符位置转换为 token 位置。由于有时候开始或结束位置的字符可能无法直接转换为 token 位置，因此使用了一些尝试和捕获异常的机制。
    # 如果 beg_pos 或 end_pos 仍旧为 None，则跳过该范围的处理，以确保不会因未找到 token 位置而导致错误。
    # 4填充正样本关联的 tokens：如果找到合法的 beg_pos 和 end_pos，则在 positive_map 的对应区域填充1，表示该范围内的 token 与当前正样本相关联。
    ...
    # 在填充完成后，对每一行进行归一化处理。具体操作是对每一行的所有 token 的值求和，并进行除法操作。加入 1e-6 是为了避免除以零。
